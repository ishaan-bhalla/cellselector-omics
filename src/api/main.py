...
from config.config import CELL_LINE_LOOKUP, MASTER_MERGED, OUTPUTS_DIR, PARQUET_DIR
from api.models import AgenticRequest, ClassicalRequest
from src.models.agentic.growth_properties import get_growth_properties
...
from src.models.classical.scorer import classify_gene, get_gene_role
from src.models.classical.similarity import DATASET_CITATIONS, find_alternatives
from src.models.graph.queries import (
    graph_to_json,
...
async def _warm_learned_weights() -> dict:
    ...
    weights fit on an old gene set. Regenerate with:
        ...
        python3 -c "
        from src.models.classical.weights_learned import optimise_weights_by_class
        import json
    ...
    async with _learned_weights_lock:
        if _learned_weights_by_class_cache is None:
            import src.models.classical.ranker as _ranker
...
def _apply_exclusion_penalties(
    ...
    from src.models.classical.scorer import load_mappings, score_rna_expression
...
async def recommend_classical(body: ClassicalRequest):
    ...
    if body.additional_genes:
        from src.models.classical.multi_gene_ranker import rank_multi_gene
        from src.models.classical.similarity import find_alternatives_multi_gene
...
async def _recommend_agentic_impl(body: AgenticRequest):
    ...
    if body.additional_genes:
        from src.models.agentic.pipeline import run_multi_gene
    ...
    # exclude_genes-vs-gene conflict check above already covers this case
    # (all_query_genes == [body.gene] when additional_genes is empty).
    from src.models.agentic.pipeline import run as pipeline_run
